...
export function generateWorkout(args) {
    const { muscleGroups, poison: workout, goal } = args
    let exer = Object.keys(exercises);
    exer = exer.filter((key) => exercises[key].meta.environment !== "home");
    let includedTracker = [];
    let numSets = 5;
    let listOfmuscleGroups;

    if (workout === "individual") {
        listOfmuscleGroups = muscleGroups;
    } else {
        listOfmuscleGroups = WORKOUTS[workout][muscleGroups[0]];
    }

    listOfmuscleGroups = new Set(shuffleArray(listOfmuscleGroups));
    let arrOfmuscleGroups = Array.from(listOfmuscleGroups);
    let scheme = goal
    let sets = SCHEMES[scheme].ratio
        ...
        .reduce((acc, curr, index) => {
            const muscleGroupToUse =
                index < arrOfmuscleGroups.length
                    ? arrOfmuscleGroups[index]
                    : arrOfmuscleGroups[index % arrOfmuscleGroups.length];
            return [
                ...acc,
                {
                    setType: curr,
    ...
    const { compound: compoundExercises, accessory: accessoryExercises } =
        exer.reduce(
            (acc, curr) => {
                ...
                for (const musc of exercises[curr].muscleGroups) {
                    if (listOfmuscleGroups.has(musc)) {
    ...
    const genWOD = sets.map(({ setType, muscleGroup }) => {
        ...
        const filteredObj = Object.keys(data).reduce((acc, curr) => {
            if (
                ...
                !data[curr].muscleGroups.includes(muscleGroup)
            ) {
